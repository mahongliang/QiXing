...
<body>
		...
		<tbody>
			<c:forEach items="${machines}" var="item">
				<tr>
					...
					<td align="center">${item.getFirstGrade()}</td>
					<td align="center">${item.getSecondGrade()}</td>
					<td align="center">${item.getThirdGrade()}</td>
	...
	<c:if test="${curPage != 1}">
		<a href="listSelect?curPage=1">首页</a>
		<a href="listSelect?curPage=${curPage - 1}">上一页</a>
	</c:if>
	...
	<c:if test="${curPage != totalPage}">
		<a href="listSelect?curPage=${curPage + 1}">下一页</a>
		<a href="listSelect?curPage=${totalPage}">尾页</a>
	</c:if>
